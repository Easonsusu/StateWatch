...
The repository now includes a runnable Xcode project, HealthKit read-permission handling, a local-only HealthKit data fetcher, baseline calculation, rule-based scoring engines, XCTest coverage, CI, a debug-only HealthKit Scoring Preview, and a debug-only Visual Dashboard Preview. The production dashboard intentionally still uses mock `StateAssessment` data while HealthKit-derived scoring and future visual direction are validated separately.
...
- Production iPhone dashboard: mock-backed by default.
- Production watch dashboard: mock-backed by default.
- HealthKit permission flow: implemented for read-only access.
- HealthKit data fetching: implemented locally for recent Apple Health samples.
- Baseline and scoring logic: implemented as local rule-based wellness estimates.
...
StateWatch is still pre-release software. Real-device HealthKit QA, local persistence, App Store polish, and production HealthKit dashboard rollout are still planned work.
...
- Mock sample data for previews, tests, and the default dashboards.
...
- Debug-only Visual Dashboard Preview for reviewing the future design-system direction without changing production dashboard behavior.
- Settings routes for Apple Health permissions, the debug scoring preview, and the debug visual dashboard preview.
- Phase 5.7 Figma-backed design-system planning and high-fidelity handoff documentation.
- SwiftUI design-system token and component foundation.
- Manual QA checklist and stabilization tests.
- GitHub Actions CI for iOS build, iOS tests, and watchOS build.

## Not Implemented / Intentionally Deferred

- The production dashboard is not wired to HealthKit-derived scoring yet.
- The production dashboard has not been redesigned with the new visual design system yet.
- HealthKit-derived scoring is not persisted yet.
- Local persistence for snapshots and baselines is not implemented yet.
- Real-device HealthKit QA is not complete.
- No networking, upload path, server sync, login, subscriptions, or API backend is implemented.
- No AI features are implemented.
- No HealthKit write access is requested.
- No advertising, marketing, profiling, or data-mining use of HealthKit data is allowed.
- No WidgetKit complication target has been added yet.
- App Store release assets, final onboarding polish, and final privacy review remain future work.
...
- HealthKit data must not be sold or shared for advertising, marketing, profiling, or data mining.
...
4. Build and run. The app should launch to the mock dashboard.
...
Manual testing is still required for real-device HealthKit permission behavior and Apple Health data availability.
